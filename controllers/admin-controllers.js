...
const createOrderReg = async (req, res, next) => {
  ...
  try {
    promoReg = await Promo.findById('600836667f3a50237e1b6643');
  } catch (err) {
    return next(
  ...
  const newOrder = new Orderreg({
    ...
    promo: promoReg.promoReg,
    totalPrice:
      parsedSlot * parsedPricePerSlot -
      (promoReg.promoReg * parsedSlot * parsedPricePerSlot) / 100,
  });
...
const createOrderBC = async (req, res, next) => {
  ...
  try {
    promoBC = await Promo.findById('600836667f3a50237e1b6643');
  } catch (err) {
    return next(
      new HttpError('failed fetching promo. Please try again later', 500)
    );
  ...
  const newOrder = new Orderbc({
    ...
    amount: parsedAmount,
    price: parsedPrice,
    promo: promoBC.promoBC,
    totalPrice:
      parsedAmount * parsedPrice - (promoBC * parsedAmount * parsedPrice) / 100,
  });
...
const getPromo = async (req, res, next) => {
  const promoId = '600836667f3a50237e1b6643';

  let foundPromo;
  try {
    foundPromo = await Promo.findById(promoId);
  } catch (err) {
    return next(
  ...
  if (!foundPromo) {
    return next(new HttpError('Promo not found', 404));
  }
...
const updatePromo = async (req, res, next) => {
  ...
  let foundPromo;
  try {
    foundPromo = await Promo.findById('600836667f3a50237e1b6643');
  } catch (err) {
  ...
  foundPromo.promoReg = promoReg ? promoReg : foundPromo.promoReg;
  foundPromo.promoBC = promoBC ? promoBC : foundPromo.promoBC;

  try {
    const sess = await mongoose.startSession();
    sess.startTransaction();
    await foundPromo.save({ session: sess });
    await sess.commitTransaction();
  } catch (err) {
    const error = new HttpError(
